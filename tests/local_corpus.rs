//! Optional historical corpus audit; excluded from normal tests with explicit ignores.
...
fn corpus_root() -> std::path::PathBuf {
    std::env::var_os("HARMONICA_TEST_CORPUS")
        .map(std::path::PathBuf::from)
        .expect("Set HARMONICA_TEST_CORPUS to the complete archived corpus directory")
}

#[test]
#[ignore = "requires an explicitly supplied private historical corpus"]
fn seed_song_selects_original_bassoon_melody() {
    let path = corpus_root().join("欢乐颂.mid");
...
fn scarborough_current_ranker_selects_flute_despite_stale_catalog_default_report() {
    let path = corpus_root().join("斯卡布罗集市.mid");
    let (parts, names) = read_midi(&path).unwrap();
