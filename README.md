...
当前版本为 **2.1.23**，实际版本以 [Cargo.toml](Cargo.toml) 为准。项目仍处于迁移验收阶段。`verification/` 中的日志和截图仅保存在本地，不随 GitHub 源码分发；自动检查通过不代表所有设备与界面场景都已验收。

曲谱中的超音域音符以圆角空心框显示，可选中、拖动、调整时长和删除，支持撤销与重做。移入可演奏音域后转为实心音符；仍在音域外时不参与试听、演奏或 MIDI 音符导出。修改随工程保存。“适配音域”只考虑实心音符，空心框可上下滚动查看。旧工程若未保存超音域音符的位置，需要重新提取后才能显示。

手机遥控曲谱按设备完整像素倍率绘制，不再限制为 2 倍。平板竖屏显示宽幅曲谱和双列曲库，横屏显示播放区与曲库双栏；横屏曲目列表在限定高度内独立滚动，搜索栏保留在列表上方，曲目数量不会无限撑长页面。旋转屏幕或改变浏览器缩放后会更新画布。已进行浏览器尺寸模拟与像素倍率逻辑验证，真实手机、平板的帧率和局域网表现仍需设备实测。

## 当前功能

桌面启动自动记录选歌各阶段耗时到 `data/logs/performance-*.jsonl`，便于定位手机指令排队、曲谱解析、试听生成或界面接收结果的延迟。记录不包含曲名、路径或遥控凭据，详见[选歌卡顿日志](docs/performance.md)。

- **声部推荐**：按 0–100 推荐指数排序，默认选择第一项，显示完整音域、音符数量和时长。推荐综合起音频率、带连奏容差的单音程度、分段活跃度及伴奏线索；旋律分数与当前参数的音域保留率分开计算，仍可手动换声部。见[算法与前后对照](docs/recommendation.md)。
...
- **遥控演奏提示**：手机开始游戏演奏时不再在电脑屏幕显示倒计时浮层；仍保留 3 秒准备时间和手机状态提示，F6 启动时保留屏幕倒计时。
...
桌面启动会自动请求管理员权限，请允许 Windows 权限提示；取消后不继续启动。已提升权限时直接打开界面。桌面主程序每次启动自动设置“高于正常”进程优先级，适用于界面、选歌加载和转换等线程；设置失败仍可启动，并在 data/logs/startup-priority-error.log 记录错误。命令行转换、诊断及成品自测不要求提升权限，也不主动提高优先级。游戏期间选歌延迟是否改善仍需实际对照测试。
...
2.1.17 修复了浏览器预连接空闲超时后，服务端提前返回错误 JSON 的问题：此时关闭连接，等待浏览器重新连接；请求格式错误仍会返回明确提示。已通过本机 TCP 回归测试，具体平板和无线网络环境仍需实测。
