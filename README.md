...
当前版本为 **2.1.15**，实际版本以 [Cargo.toml](Cargo.toml) 为准。项目仍处于迁移验收阶段。`verification/` 中的日志和截图仅保存在本地，不随 GitHub 源码分发；自动检查通过不代表所有设备与界面场景都已验收。
...
手机遥控曲谱按设备完整像素倍率绘制，不再限制为 2 倍。平板竖屏显示宽幅曲谱和双列曲库，横屏显示播放区与曲库双栏；旋转屏幕或改变浏览器缩放后会更新画布。已进行浏览器尺寸模拟与像素倍率逻辑验证，真实手机、平板的帧率和局域网表现仍需设备实测。
