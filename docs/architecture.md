...
控制器的 `state`、`preferences`、`library` 字段为私有；外部通过 `state()`、`preferences()` 和 `library()` 读取共享引用，不提供可变访问器。更改必须通过业务方法完成校验、版本更新、导出/快照失效和保存。需要构造异常内部状态的回归测试位于控制器私有测试模块；公开接口测试仍作为集成测试。曲库诊断在创建控制器前准备临时偏好，计时区只发起一次扫描。
...
2. `score/recommendation.rs` 计算 0–100 推荐指数；`service::load_ranked_midi` 将排序结果与声部数据交给控制器。特征集中在 `score/part_features.rs`，考虑带尾部容差的单音程度、起音频率、分段活跃度、相对音区及音高/节奏周期，不修改原始时间。指数是旋律偏好，不混入假定最佳移调的可达性。详见[算法说明与固定样本对照](recommendation.md)。
3. 桌面导入页按推荐顺序展示音符数、完整音域、时长与推荐指数，默认选择第一项，允许手动改选。
4. `score/melody.rs` 的 `fit_part` 根据选定声部和选项提取旋律，执行速度、移调、口琴音域及乐句八度处理；`prepare` 与 CLI `inspect` 共用它，报告提取后与原声部两种分母的保留率。自动八度适配不保证保留所有音符，界面需要区分调整与丢弃统计。
...
`ScriptPlayer` 使用原子替换写入 `command.json`，命令包含 `id`、`action` 和可选 `start_ms`。Rust 用专用结构体固定输出顺序，兼容已导出、要求 `id` 在前的旧脚本；新 AHK 解析器按字段识别，不依赖顺序，拒绝重复字段、未知字段和非法值。编号为 1–9007199254740991，起点为 0–86400000 整数毫秒；整条命令校验通过后才消耗编号，旧编号不会重复执行。读取命令时允许共享删除，避免阻挡 Rust 原子替换。Rust 对命令写入的 Windows 访问/共享冲突按 5 毫秒间隔重试，重试窗口最多 100 毫秒；超时或其他错误仍返回失败，写入成功后才递增命令编号。此策略仅用于演奏命令，不改变工程保存的原子写入流程。

AHK 发布包含 `request_id` 的状态，并每 100 毫秒定时重新发布，使短暂文件占用后状态能恢复；测试夹具也保留相同的定时发布行为。Rust 只接受当前编号的有效状态。`exit.stop` 退出信号优先于命令文件；F6/F8 是独立热键路径。Windows 下的 `src/media/playback/protocol_tests.rs` 让真实 Rust 命令写入、AHK 解析与状态发布、Rust 状态读取联动，仅替换会操作游戏的开始/停止处理函数，不注册热键或发送按键；这与假后端测试、实际游戏验收是不同证据。
