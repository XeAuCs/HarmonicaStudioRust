...
/// Shared repaint state for one or more demand-driven canvases.
///
/// A new invalidator starts invalidated. Calling [`invalidate`](Self::invalidate) coalesces with
/// any pending repaint.
#[derive(Clone)]
pub struct Invalidator(Rc<Cell<u64>>, Rc<RefCell<Vec<Rc<dyn Fn() -> bool>>>>);
...
impl Invalidator {
    ...
    pub fn invalidate(&self) {
        ...
        let live = callbacks
            ...
            .collect::<Vec<_>>();
        *self.1.borrow_mut() = live;
    }

    fn subscribe(&self, callback: Rc<dyn Fn() -> bool>) {
        self.1.borrow_mut().push(callback);
    }
}
...
impl Component for CanvasHost {
    ...
    fn create(input: &Self::Input, _context: &ComponentContext<Self>) -> Self {
        let runtime = Rc::new(CanvasRuntime {
        ...
        subscribe_invalidator(&runtime, &input.invalidator);
        Self { runtime }
    }
    ...
    fn input_changed(&mut self, input: &Self::Input, _context: &ComponentContext<Self>) {
        ...
        *self.runtime.input.borrow_mut() = input.clone();
        subscribe_invalidator(&self.runtime, &input.invalidator);
...
fn subscribe_invalidator(runtime: &Rc<CanvasRuntime>, invalidator: &Invalidator) {
    ...
    invalidator.subscribe(callback);
}
    ...
    fn handle_event(self: &Rc<Self>, event: SwapChainPanelEvent) {
        match event {
            ...
            SwapChainPanelEvent::Rendering => {
                ...
                let should_render =
                    input.mode == RenderMode::Continuous
                        || input.invalidator.0.get() != self.rendered_generation.get();
                drop(input);
                if should_render {
...
impl CanvasRuntime {
    ...
    fn render_frame(self: &Rc<Self>) {
        ...
        let generation = invalidator.0.get();
        if mode == RenderMode::Demand && generation == self.rendered_generation.get() {
            return;
        }
...
mod tests {
    ...
    fn metric_changes_invalidate_demand_rendering() {
        ...
        for next in [
            ...
            assert_ne!(invalidator.0.get(), 0);
        }
    }

    #[test]
    fn invalidation_wakes_all_live_demand_canvases() {
        let invalidator = Invalidator::new();
        invalidator.0.set(0);
        ...
        let first_callback: Rc<dyn Fn() -> bool> = Rc::new(move || {
            first_count.set(first_count.get() + 1);
            true
        });
        let second_callback: Rc<dyn Fn() -> bool> = Rc::new(move || {
            second_count.set(second_count.get() + 1);
            true
        });
        invalidator.subscribe(first_callback);
        invalidator.subscribe(second_callback);
