...
#[test]
fn component_dialog_siblings_require_native_children_owner_before_component_slot() {
    struct DialogRoot;
    impl Component for DialogRoot {
        ...
        fn view(&self, input: &Self::Input, context: &mut ViewContext<Self>) -> View {
            let (native_owner, settings_open, phone_open) = *input;
            let main = Grid::new().children((TextBlock::new().text("Main"),));
            let settings = ContentDialog::new()
                .is_open(settings_open)
                .on_closed(context.callback(|_| ()))
                .content(if settings_open {
                    Border::new().content(TextBlock::new().text("Settings"))
                } else {
                    View::empty()
                });
            let phone = ContentDialog::new()
                .is_open(phone_open)
                .on_closed(context.callback(|_| ()))
                .content(if phone_open {
                    Border::new().content(TextBlock::new().text("Phone"))
                } else {
                    View::empty()
                });
            if native_owner {
                ...
                View::fragment((main, settings, phone))
            }
        }
    }
    // The component inserts a Slot between its rendered root and the Window.
    // A fragment cannot supply the native Children owner required by dialogs.
    for (settings, phone) in [(false, false), (true, false), (false, true)] {
        let mut invalid = Pump::new(RecordingRuntime::default());
        assert_eq!(
            invalid.mount_view(View::component::<DialogRoot>((false, settings, phone))),
            Err(PumpError::StructureUnsupported)
        );
        assert_eq!(invalid.root(), None);
        assert!(invalid.runtime().is_empty());
    }
    let mut pump = Pump::new(RecordingRuntime::default());
    pump.mount_view(View::component::<DialogRoot>((true, false, false)))
        .unwrap();
    let dialogs = dialog_nodes(pump.runtime());
    assert_eq!(dialogs.len(), 2);
    let window = pump.window().unwrap();
    assert_eq!(pump.runtime().node(window).unwrap().children().len(), 1);
    for (settings, phone) in [(true, false), (false, false), (false, true), (false, false)] {
        pump.update_view(View::component::<DialogRoot>((true, settings, phone)))
            .unwrap();
        assert_eq!(dialog_nodes(pump.runtime()), dialogs);
        for (node, open) in dialogs.iter().copied().zip([settings, phone]) {
            let state = pump.runtime().content_dialog(node).unwrap();
            assert_eq!(state.desired_open, open);
            assert_eq!(
                pump.runtime().node(node).unwrap().children().is_empty(),
                !open
            );
            if !open && state.pending {
                let revision = pump
                    .event_revision(node, EventId::ContentDialogClosed)
                    .unwrap();
                pump.runtime_mut().complete_content_dialog(
                    node,
                    revision,
                    ContentDialogResult::None,
                );
                // Declarative hide suppresses the user-initiated closed callback.
                assert_eq!(pump.dispatch_events().unwrap(), 0);
                pump.dispatch_components(1).unwrap();
            }
        }
    ...
    for node in dialogs {
        let state = pump.runtime().content_dialog(node).unwrap();
        assert!(!state.desired_open && !state.pending && !state.queued);
