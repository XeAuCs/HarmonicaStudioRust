// Modified for Harmonica Studio on 2026-09-19; see third_party/windows-rs/SOURCE.md.
use std::any::Any;
...
) -> impl Fn(windows_core::Ref<windows_core::IInspectable>, windows_core::Ref<PointerRoutedEventArgs>)
{
    move |_, args| {
        ...
        if let Some(args) = args.as_ref() {
            info.modifiers = input_modifiers().unwrap_or_default();
            if let Ok(point) = args.GetCurrentPoint(&element) {
                ...
                if let Ok(properties) = point.Properties() {
                    info.wheel_delta = properties.MouseWheelDelta().unwrap_or(0);
                    info.is_horizontal_wheel = properties.IsHorizontalMouseWheel().unwrap_or(false);
        ...
        if phase == PointerEventPhase::Press {
            info.capture_succeeded = match sink.apply_pointer_press_policy(
                node,
                &element,
                windows_core::InRef::from(args.as_ref()),
            ) {
                Ok(value) => value,
                Err(error) => {
        ...
        // Publish the release before dropping capture. Releasing a pointer can synchronously
        // raise PointerCaptureLost; if that event is queued first it cancels the drag before
        // PointerReleased reaches the component and the final seek/edit is discarded.
        if phase == PointerEventPhase::Release {
            sink.enqueue(node, event, revision, EventPayload::PointerEventInfo(info));
            if let Err(error) =
                sink.apply_pointer_release_policy(
                    node,
                    event,
                    revision,
                    &element,
    ...
    fn set_resource_overrides(
        ...
        for (key, value) in resources.values() {
            ...
            let value: windows_core::IInspectable = match value {
                ResourceValue::Double(value) => windows_reference::IReference::from(*value).into(),
                ResourceValue::FontFamily(value) => {
                    let escaped = value
                        .replace('&', "&amp;")
                        .replace('<', "&lt;")
                        .replace('>', "&gt;");
                    let xaml = format!(
                        "<FontFamily xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'>{escaped}</FontFamily>"
                    );
                    XamlReader::Load(&xaml).map_err(native_error)?
                }
                ...
                ResourceValue::Thickness(value) => {
                    // ContentDialog templates also require native boxed geometry values.
                    XamlReader::Load(&format!(
                        "<Thickness xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'>{},{},{},{}</Thickness>",
                        value.left(), value.top(), value.right(), value.bottom()
                    )).map_err(native_error)?
                }
                ResourceValue::CornerRadius(value) => {
                    // WinUI theme conversion requires its native boxed CornerRadius.
                    // StockReference passes direct property reads but fails during live template loading.
                    XamlReader::Load(&format!(
                        "<CornerRadius xmlns='http://schemas.microsoft.com/winfx/2006/xaml/presentation'>{},{},{},{}</CornerRadius>",
                        value.top_left(), value.top_right(), value.bottom_right(), value.bottom_left()
                    )).map_err(native_error)?
                }
            };
                ...
                if matches!(
                    *property,
                    PropertyId::ButtonResources
                        | PropertyId::BorderResources
                        | PropertyId::ContentDialogResources
                ) {
                    return match value {
