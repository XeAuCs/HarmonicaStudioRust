...
pub fn set_property(
    ...
    match (handle, property, value) {
        ...
        (
            ...
            let family = bindings::XamlReader::Load(&xaml)
                .and_then(|value| value.cast::<bindings::IFontFamily>())
                .map_err(native_error)?;
            control.SetFontFamilyObject(&family).map_err(native_error)
        }
        ...
        (
            ...
            PropertyValue::F64(value),
        ) => control.SetSmallChange(*value).map_err(native_error),
...
pub fn clear_property(handle: &Handle, property: PropertyId) -> Result<(), RuntimeError> {
    match (handle, property) {
        ...
        (Handle::TextBlock(_), PropertyId::TextBlockFontFamily) => {
            clear_value(handle, bindings::TextBlock::FontFamilyProperty)
        }
        ...
        (Handle::NumberBox(_), PropertyId::NumberBoxSmallChange) => {
            clear_value(handle, bindings::NumberBox::SmallChangeProperty)
        }
...
pub fn subscribe_event(
    ...
    match (handle, event) {
        ...
        .map_err(native_error),
        (Handle::Border(value), EventId::BorderPointerWheelChanged) => {
            let source = value.cast::<IUIElement>().map_err(native_error)?;
            source.PointerWheelChanged(pointer_event_handler(
                sink,
                node,
