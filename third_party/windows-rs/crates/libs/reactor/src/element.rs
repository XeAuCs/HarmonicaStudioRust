...
pub struct PointerEventInfo {
    ...
    /// Raw wheel delta; 120 is one standard detent.
    pub wheel_delta: i32,
    pub is_horizontal_wheel: bool,
...
pub enum ResourceValue {
    /// A finite non-negative dimension or font size.
    Double(f64),
    /// A native FontFamily source.
    FontFamily(String),
    Color(Color),
    Thickness(Thickness),
...
impl ResourceOverrides {
    ...
    pub fn set(mut self, key: impl Into<String>, value: impl Into<ResourceValue>) -> Self {
        ...
        match &value {
            ResourceValue::Double(value) => assert!(
                value.is_finite() && *value >= 0.0,
                "resource dimension must be finite and non-negative"
            ),
            ResourceValue::FontFamily(value) => {
                assert!(!value.is_empty(), "resource font family must not be empty")
            }
